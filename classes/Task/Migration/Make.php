 ...
 *
 *  # The migration will be created in `modules/myapp/database/migrations/myapp/`
 ...
 */
class Task_Migration_Make extends Minion_Task {
	...
	public function generate($options, $up = NULL, $down = NULL)
	{
		...
		$location = rtrim(realpath($options['location']), '/').'/database/migrations/';
		...
		.View::factory('minion/task/migration/make/template')
			->set('class', $class)
