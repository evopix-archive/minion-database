...
class Model_Minion_Migration extends Model
{
	...
	{
		$this->_db = $db;
		$this->_table = Kohana::$config->load('minion/database')->table;
	}

	/**
	 * Returns a list of migrations that are currently installed
	 *
	 * @return array
	 */
	public function installed_migrations()
	{
		$installed = [];
		foreach ($this->fetch_all() as $migration)
		{
			$installed[$migration['group'].':'.$migration['timestamp']] = $migration;
		}
		return $installed;
	}
	...
	{
		$files = Kohana::list_files('database/migrations');
	...
	public function get_migration_from_filename($file)
	{
		...
		$migration['group'] = dirname(substr($file, 20, -strlen(EXT)));
	...
	{
		$schema = $this->_get_schema_builder();
		if ( ! $schema->has_table($this->_table))
		{
			$schema->create($this->_table, function($table)
			{
				/* @var $table Schema_Blueprint */
				$table->string('timestamp', 14);
				$table->string('description', 100);
				$table->string('group', 100);
				$table->tiny_integer('applied')->default(0);
				$table->primary(['timestamp', 'group']);
				$table->unique(['timestamp', 'description']);
			});
		}
	}
	...
	/**
	 * Selects all migrations from the migrations table
	 *
	...
	public function fetch_current_versions($key = 'group', $value = NULL)
	{
		...
		foreach ($this->fetch_groups() as $group)
		{
			...
			{
				// Indexed rows
				$current_versions[] = $migration;
			}
			elseif ($key === NULL)
			{
				// Indexed columns
				$current_versions[] = $migration[$value];
			}
			elseif ($value === NULL)
			{
				// Associative rows
				$current_versions[$migration[$key]] = $migration;
			}
			else
			{
				// Associative columns
				$current_versions[$migration[$key]] = $migration[$value];
			}
		}
	...
	public function get_migration_id($migration)
	{
		return $migration['group'].':'.$migration['timestamp'];
	}
	...
	protected function _get_schema_builder()
	{
		return new Schema_Builder($this->_db);
	}
